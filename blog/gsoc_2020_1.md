...
Let's revisit our encoding example from before and have a look how the v0.1 looks like:
...
Next, one can configure the encoder by calling one of various methods like `setDim(...)`. What's actually interesting about this is the function signature. Because we are having `&mut self` and also returning `Self` (by calling `self.clone()`) we enable trailing the methods, but without invalidating the object the method is called on.
...
##### JavaScript
And that is how it looks like from a js-user perspective:
```ts
...
### Next steps
That's what I achieved so far. Looking back it is actually something and I am very grateful getting the opportunity to work on this thanks to GSoC :)
